...
CREATE TABLE `photos` (
    ...
    FOREIGN KEY (media_id) REFERENCES media_types(id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
-- таблица комментариев под различными медиа пользователя
DROP TABLE IF EXISTS `comments`;
CREATE TABLE `comments` (
	id SERIAL PRIMARY KEY,
	from_user_id BIGINT UNSIGNED NOT NULL,
	to_user_id BIGINT UNSIGNED NOT NULL,
	to_media_type_id BIGINT UNSIGNED NOT NULL,
	body TEXT,
	created_at DATETIME DEFAULT NOW(),

    FOREIGN KEY (from_user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (to_user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (to_media_type_id) REFERENCES media(id) ON UPDATE CASCADE ON DELETE CASCADE
);

-- таблица с друзьями пользователя
...
CREATE TABLE `friends` (
    ...
    FOREIGN KEY (main_user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE
);

-- таблица с обновлениями, которые произошли у друзей
...
CREATE TABLE `news` (
	...
	FOREIGN KEY (from_id) REFERENCES friends(friend_id) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (media_type_id) REFERENCES media(id) ON UPDATE CASCADE ON DELETE CASCADE
);


-- музыкальный плейлист пользователя
...
CREATE TABLE `musics` (
	...
	FOREIGN KEY (media_id) REFERENCES media(id) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (playlist_id) REFERENCES music_playlist(id) ON UPDATE CASCADE ON DELETE CASCADE
);

-- таблица с постами на своей стене (скорей всего нужна отдельная таблица со стеной)
...
CREATE TABLE `posts` (
	...
	FOREIGN KEY (post_media_id) REFERENCES media_types(id) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (post_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE
);
